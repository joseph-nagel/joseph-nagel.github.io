...
In this post, we will just briefly cover the basic concepts of restricted Boltzmann machines for unsupervised learning. See [[Hinton, 2012](https://doi.org/10.1007/978-3-642-35289-8_32); [Fischer and Igel, 2012](https://doi.org/10.1007/978-3-642-33275-3_2); [Fischer and Igel, 2014](https://doi.org/10.1016/j.patcog.2013.05.025)] for more detailed introductions.
...
as the distribution over the visible units. One also finds that the hidden units given the visible ones are conditionally independent, and vice versa. Consequently, one has $$q_{\boldsymbol{\theta}}(\boldsymbol{h} \vert \boldsymbol{v}) = \prod_{j=1}^S q_{\boldsymbol{\theta}}(h_j \vert \boldsymbol{v})$$ and $$q_{\boldsymbol{\theta}}(\boldsymbol{v} \vert \boldsymbol{h}) = \prod_{i=1}^R q_{\boldsymbol{\theta}}(v_i \vert \boldsymbol{h})$$. The derivation uses $$h_j \in \{0, 1\}$$. Furthermore, one can show that
